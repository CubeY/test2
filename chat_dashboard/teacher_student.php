...
$avoid_teacher_selection = "true";

 if($_SESSION['user_role'] == 'viewer') { 	echo '<h3>You are restricted for this page!</h3>';exit;} 
...
{
	$slotID = $_REQUEST['slot'];
	$slotDtl = Student::slotDtl($slotID);
	//print_r($slotDtl);
}
else
{
...
<h3 class="hd-title"><span style="margin:0 10px;"> <?php User::face_photo($teacher_id,$thumb = true , $w = 30 , $h = false)?></span><?php if($slotID) { ?>Slot: <?php echo $slotDtl['start_time'].'&nbsp;To&nbsp;'.$slotDtl['end_time']; } ?>&nbsp;&nbsp;&nbsp; Student List</h3> 
...
<a href="#" onclick="showAddNewUrsFrm()">Add New Student</a>
</span>
...
<?php if($slotID != '') {?><input type="hidden" name="slot_id" id="slot_id"  value="<?php echo $slotID;?>"/><?php } ?>
...
<div class="create_new_cls" >
 ...
 // print_r($all_user);
  for($i_user = 0;$i_user < count($all_status);$i_user++)
  {
    ...
    <div class="clear"></div>
     <div class="single_row">
        <div class="label_u">Date of Birth:</div>
        <div>
