...
error_reporting(E_ALL & ~E_STRICT);
 ...
 class User  {
    ...
    <div class="single_row">
        ...
        <div><input type="text" class="required email" name="u_email" value="<?php echo $row['u_email'];?>" style="width:300px;" />
        <label for="u_email" style="display:none;" class="error"></label></div>
    </div>
    <div class="clear"></div>
    <div class="single_row">
   ...
   <div class="clear"></div>
	...
	public function update_user_dtl()
	{
		...
		else if(User::is_username_exist($user,$uid) == true)
		{
	...
	public function is_email_exist($email,$uid = false)
	{
		$db = Database::init(DB_ACCOUNTS_HOST, DB_ACCOUNTS_USER, DB_ACCOUNTS_PWD, DB_ACCOUNTS_NAME);
		$sql="SELECT email FROM user_master WHERE email='".$email."'";
		if($uid !='')
		$sql .=" AND id != ".$uid."";
		//echo $sql;
		$rs = $db->doQuery($sql);
	...
	public function is_username_exist($username,$uid = false)
	{
		$db = Database::init(DB_ACCOUNTS_HOST, DB_ACCOUNTS_USER, DB_ACCOUNTS_PWD, DB_ACCOUNTS_NAME);
		$sql="SELECT user FROM user_master WHERE user ='".$username."'";
		if($uid !='')
		$sql .=" AND id != ".$uid."";
		//echo $sql;
		$rs = $db->doQuery($sql);
   ...
   <?php 
	...
	public function total_after_FT_users_by_teacher_id($teacher_id = false)
	{
		...
		/*if($teacher_id)
		{
			$sql2 = "SELECT count(users_list.id) as total FROM users_list join teacher_slots on users_list.slot=teacher_slots.id WHERE users_list.status IN (4,6)";
			...
			$sql3 = "SELECT count(users_list.id) as total FROM users_list WHERE users_list.converted_by = ".$teacher_id." AND users_list.status IN (7,8) ";
			$q3 = $acdb->doQuery($sql3);
			$res3 = mysql_fetch_assoc($q3);
			//print_r($res2);
			$all_total =  $total + $res3['total'];
		...
		return  $res2['total'];
			...
			$sql3 = "SELECT count(users_list.id) as total FROM users_list WHERE users_list.converted_by > 0 AND users_list.status IN (7,8) ";
			if($teacher_id)
			$sql3 .= "AND users_list.converted_by = ".$teacher_id;
			$q3 = $acdb->doQuery($sql3);
			$res3 = mysql_fetch_assoc($q3);
			//print_r($res2);
			$all_total =  $total + $res3['total'];
	...
	public function total_responsive_after_FT_users_by_teacher_id($teacher_id = false)
	{
	    $acdb = Database::init(DB_ACCOUNTS_HOST, DB_ACCOUNTS_USER, DB_ACCOUNTS_PWD, DB_ACCOUNTS_NAME);
			...
			$sql2 = "SELECT count(users_list.id) as total FROM users_list join teacher_slots on users_list.slot=teacher_slots.id WHERE users_list.status IN (6)";
			if($teacher_id)
			$sql2 .= "AND teacher_slots.teacher_id = ".$teacher_id;
			$q2 = $acdb->doQuery($sql2);
			$res2 = mysql_fetch_assoc($q2);
	...
	public function converted_users_by_teacher_id($teacher_id = false)
	{
	    $acdb = Database::init(DB_ACCOUNTS_HOST, DB_ACCOUNTS_USER, DB_ACCOUNTS_PWD, DB_ACCOUNTS_NAME);
			...
			$sql2 = "SELECT count(users_list.id) as total FROM users_list WHERE users_list.converted_by > 0 AND users_list.status IN (7,8) ";
			if($teacher_id)
			$sql2 .= "AND users_list.converted_by = ".$teacher_id;
			$q2 = $acdb->doQuery($sql2);
			$res2 = mysql_fetch_assoc($q2);
			//print_r($res2);
			return  $res2['total'];
	...
	public function calculate_churn_in_array($datef,$funnel = false)
	{
	    $db = Database::init(DB_ACCOUNTS_HOST, DB_ACCOUNTS_USER, DB_ACCOUNTS_PWD, DB_ACCOUNTS_NAME);
		$res = array();
		$total = 0;
	...
	public function calculate_churn_paying_in_array($datef,$funnel = false)
	{
	    $db = Database::init(DB_ACCOUNTS_HOST, DB_ACCOUNTS_USER, DB_ACCOUNTS_PWD, DB_ACCOUNTS_NAME);
		$res = array();
		$total = 0;
	...
	public function teacher_true_conversion_rate($t_paying_user,$t_cancelled,$t_avl_url_shown,$teacher_id)
	{
		//" true conversion" = (Paying+cancelled) / LINE URL shown
		
		//$total = $t_paying_user+$t_cancelled;
		$total = User::converted_users_by_teacher_id($teacher_id);
		if($total <= 0) return '0 %';
	...
	public function overall_true_conversion_rate()
	{
		//" true conversion" = (Paying+cancelled) / LINE URL shown
		
		//$total = $t_paying_user+$t_cancelled;
		$total = User::converted_users_by_teacher_id($teacher_id = false);
		$t_avl_url_shown = User::total_num_line_url_shown();
		if($total <= 0) return '0 %';
		if($t_avl_url_shown <= 0) return '0 %';
		
		$true_cnv = round( 100*($total / $t_avl_url_shown ) , 2).'%';
		return $true_cnv.' True Conversion';
	}
	...
	public function total_num_line_url_shown()
	{
		$db = Database::init(DB_ACCOUNTS_HOST, DB_ACCOUNTS_USER, DB_ACCOUNTS_PWD, DB_ACCOUNTS_NAME);
		$sql= "SELECT COUNT(id) AS total FROM  line_url_shown ";
		$q = $db->doQuery($sql);
